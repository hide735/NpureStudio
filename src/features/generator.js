...
// Text-to-Image 生成モジュール（onnx-community/sd-turbo-onnx）
...
export async function initGenerator(transformers, device = null) {
    if (!transformers) throw new Error('transformers instance required');
    if (generator) return generator;

    const modelId = 'onnx-community/sd-turbo-onnx';
    // Rely on the explicit device parameter from initTransformers.
    // Do NOT call navigator.gpu.requestAdapter() from here.
    const dev = device ?? 'wasm';
    const dtype = 'fp16';

    log.info(`Initializing generator: ${modelId} on ${dev} (${dtype})`);
    try {
        const progressCallback = (p) => {
            try { log.info(`Generator progress: ${Math.round((p || 0) * 100)}%`); } catch (e) {}
        };

        // Attempt to create a text-to-image pipeline. Some transformer builds
        // accept a `progress_callback` option — pass it when supported.
        generator = await transformers.pipeline('text-to-image', modelId, { device: dev, dtype, progress_callback: progressCallback });
        log.info('Generator pipeline loaded');
        return generator;
    } catch (err) {
        log.error('Failed to initialize generator:', err?.message || err);
        throw err;
    }
}

/**
 * Generate an image for the given prompt and return an HTMLCanvasElement.
 * Options may include downstream pipeline overrides.
 */
export async function generateImage(prompt, options = {}) {
    if (!generator) throw new Error('generator not initialized');
    const opts = Object.assign({ num_inference_steps: 4, guidance_scale: 0.0 }, options);
    ...
    try {
        ...
        const canvas = await pipelineOutputToCanvas(out);
        return canvas;
    } catch (err) {
...
async function pipelineOutputToCanvas(output) {
    ...
    // Helper to draw an HTMLImageElement or ImageBitmap to canvas
    const drawToCanvas = (imgLike) => {
        ...
        export async function initGenerator(transformers, device = null, options = {}) {
            if (!transformers && !options.useONNX) throw new Error('transformers instance required or set useONNX:true');
            if (generator) return generator;

            // 1) Try to use transformers pipeline if available
            const modelId = options.modelId || 'onnx-community/sd-turbo-onnx';
            const dev = device ?? 'wasm';

            log.info(`Attempting to initialize generator pipeline (device=${dev})`);
            try {
                // Some transformers builds do not support text-to-image; this may throw
                const pipeline = await transformers.pipeline('text-to-image', modelId, { device: dev });
            ...
            // 2) Fallback: initialize ONNX-based generator (basic UNet prototype)
        ...
        async function createOnnxSession(ort, url) {
            // ort.InferenceSession.create accepts a URL in browser builds
            if (!ort || !ort.InferenceSession) throw new Error('Invalid ONNX runtime');
            try {
                const session = await ort.InferenceSession.create(url);
                return session;
            } catch (e) {
                // Try fetching as array buffer and create from buffer if direct URL create fails
        ...
        export async function generateImage(prompt, options = {}) {
            ...
            if (generator.type === 'pipeline') {
                // Use existing transformers pipeline behaviour
                try {
            ...
            if (generator.type === 'onnx') {
                // Simple ONNX-based generator: run the small UNet on random latent.
                ...
                // Default dimensions: [1,4,128,128]
                ...
                for (let y = 0; y < outH; y++) {
                    for (let x = 0; x < outW; x++) {
                        ...
                        const base = idx * 4;
                        // Assume outputs are in [-1,1] (tanh) or [0,1]
        ...
        function getOutValue(data, channel, C, H, W, pixelIndex) {
            // channel-first ordering: data[channel * H * W + pixelIndex]
            if (!data) return 0;
            const planeSize = H * W;
        ...
        async function pipelineOutputToCanvas(output) {
            // Reuse previous implementation for pipeline outputs
            const res = Array.isArray(output) ? output[0] : output;
            const drawToCanvas = (imgLike) => {
                const canvas = document.createElement('canvas');
                canvas.width = imgLike.width || imgLike.naturalWidth || imgLike.bitmapWidth || 512;
